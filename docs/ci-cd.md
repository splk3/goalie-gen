...
| **Test Document Generators** | `test-document-generators.yml` | PRs to any branch, manual triggers, or weekly Saturdays at 3:30 AM UTC  | Runs all CLI document generators (default + custom args), validates generated DOCX/PDF outputs, and uploads artifacts. |
...
| **Update Docs Agent**      | `update-docs-agent.lock.yml` | Manual dispatch (weekly schedule currently commented out)               | Runs an AI docs-maintenance workflow. Pull requests from this workflow are forbidden from modifying GitHub workflows. |
