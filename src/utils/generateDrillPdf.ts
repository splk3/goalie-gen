...
export const loadImageAsDataURL = (imagePath: string): Promise<{ dataURL: string; width: number; height: number }> => {
...
export const generateDrillPdf = async (drillData: DrillData, drillFolder: string): Promise<jsPDF> => {
  const doc = new jsPDF()
