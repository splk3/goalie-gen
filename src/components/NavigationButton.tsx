...
export default function NavigationButton({
  ...
  const baseClasses =
    "text-usa-white font-bold py-4 px-8 rounded-lg text-xl shadow-lg transition-colors transform hover:scale-105 text-center";
  const defaultClasses = `${baseClasses} ${variantClasses[variant]}`;

  return (
    <Link to={to} className={[defaultClasses, className].filter(Boolean).join(" ")}>
      {children}
    </Link>
  );
}
